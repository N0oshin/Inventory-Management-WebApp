...
def load_user(user_id):
    conn = get_db_connection()
    # Check for admin
    user_data = conn.execute('SELECT * FROM Admin WHERE id = ?', (user_id,)).fetchone()
    if user_data:
...
def add_user():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        conn.execute('INSERT INTO User (u_username, u_password) VALUES (?,?)',(username, hashed_password))
        conn.commit()
        conn.close()
...
def user_signin():
    ...
    if request.method == 'POST':
        ...
        user_data = conn.execute("SELECT * FROM User WHERE u_username = ?", (username,)).fetchone()
        conn.close()
    ...
    if request.method == 'POST':
        ...
        if not item_wt or float(item_wt) <= 0:
            ...
            if add < 0:
                ...
                conn.close()
                conn = get_db_connection()
                conn.execute('INSERT INTO Orders (u_id,item_id,quantity,price) VALUES (?,?,?,?)', (current_user.id, i_id, item_wt, price))
                conn.commit()
                conn.close()
...
def user_orders():
    ...
    conn = get_db_connection()
    user_orders = conn.execute('SELECT * FROM Orders inner join Items on item_id=id WHERE u_id=?', (current_user.id,)).fetchall()
    conn.close()
...
def user_history():
    conn = get_db_connection()
    orders = conn.execute('SELECT * FROM History inner join Items on item_id=id where u_id = ?', (current_user.id,)).fetchall()
    conn.close()
